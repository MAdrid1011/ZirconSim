...
template <typename T> T objectAt(const std::vector<uint8_t> &file, size_t offset) {
...
void SparseMemory::save(CheckpointWriter &writer) const {
    writer.write<uint64_t>(words_.size());
    for (const auto &[address, value] : words_) {
        writer.write(address);
        writer.write(value);
    }
}

void SparseMemory::restore(CheckpointReader &reader) {
    const uint64_t count = reader.readCount(PlatformDevices::kRamSize / sizeof(uint32_t), "memory word");
    words_.clear();
    words_.reserve(static_cast<size_t>(count));
    for (uint64_t index = 0; index < count; ++index) {
        const uint32_t address = reader.read<uint32_t>();
        const uint32_t value = reader.read<uint32_t>();
...
ElfImage ElfImage::load(const std::string &path, uint32_t raw_base) {
    ...
    ElfImage image;
    const bool magic = file.size() >= 4 && file[0] == 0x7f && file[1] == 'E' && file[2] == 'L' && file[3] == 'F';
    if (!magic) {
    ...
    if (header.ident[4] != kElfClass32 || header.ident[5] != kElfDataLittleEndian || header.machine != kMachineRiscV) {
    ...
    for (uint16_t index = 0; index < header.program_header_count; ++index) {
        const size_t offset =
            header.program_header_offset + static_cast<size_t>(index) * header.program_header_entry_size;
        ...
        const uint32_t address = program.physical_address != 0 ? program.physical_address : program.virtual_address;
    ...
    for (uint16_t index = 0; index < header.section_header_count; ++index) {
        const size_t offset =
            header.section_header_offset + static_cast<size_t>(index) * header.section_header_entry_size;
...
std::optional<int> TestExitMonitor::observeWrite(uint32_t address, uint32_t data, uint8_t strobe) {
    ...
    for (uint32_t byte = 0; byte < 4; ++byte) {
        if ((strobe & (1u << byte)) != 0 && byte + byte_offset < 4) {
            ...
            value_ = (value_ & ~mask) | (((data >> (byte * 8)) & 0xffu) << ((byte + byte_offset) * 8));
        }
    }
    if ((value_ & 1u) == 0 || value_ == 0) {
        return std::nullopt;
    }
    return value_ == 1u ? std::optional<int>(0) : std::optional<int>(static_cast<int>(value_ >> 1));
}
